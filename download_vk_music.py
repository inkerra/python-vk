...
def options():
    ...
    parser.add_argument('-p', '--path', metavar='DIR', default='.',
        help='directory path to save audios')
    parser.add_argument('-l', '--links_dir', metavar='DIR',
        default='playlist',
        help='directory to save links to saved audios in directory with saved audios')
...
def main():
    ...
    access_token, uid = auth.console_auth("audio")
    ...
    mkdir(path)
    clean_dir(os.path.join(path, links_dir))
